...
for (let i = 1; i <= NUM_RUNS; i++) {
  ...
  try {
    ...
    const command = `npx lighthouse "${TARGET_URL}" --output=json --output-path="${outputPath}" --only-categories=performance --preset=desktop --throttling.cpuSlowdownMultiplier=1 --quiet ${cacheFlag}`;
    ...
    if (i < NUM_RUNS) execSync('sleep 2');
  } catch (err) {
...
const summary = { url: TARGET_URL, timestamp: new Date().toISOString(), runs: NUM_RUNS, cold: { fcp: coldFCP, lcp: coldLCP, tti: coldTTI }, warm: { fcp: warmFCP, lcp: warmLCP }, all: { fcp: allFCP, lcp: allLCP, score: allScores }, detailedResults: results };
